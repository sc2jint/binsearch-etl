...
public class Engine implements ETLService {
    ...
    @Autowired
    @Qualifier(EngineConfiguration.FILE2TYPE3FEATURE_DATABASE_JOB_SERVICE)
    DataBaseJobService dataBaseJobService;

    @Autowired
    @Qualifier(EngineConfiguration.FILE2TYPE3FEATURE_FEATUREEXTRACT_JOB_SERVICE)
    ExtractJobService extractJobService;

    @Autowired
    @Qualifier(EngineConfiguration.FILE2TYPE3FEATURE_RECORD_SERVICE)
    ...
    @Value("#{'${file2type3feature.source.system.language}'.split(',')}")
    ...
    //抽取表数
    @Value("${file2type3feature.source.system.table.count}")
    Integer sourceTableCount;

    @Value("${file2type3feature.source.system.pageSize}")
    Integer pageSize;

    @Override
    public void init() throws Exception {
        if(CollectionUtils.isEmpty(languages)){
            throw new Exception("file2type3feature languages is empty");
        }
        ...
        if(languagesMap.isEmpty()) {
            for (String language : languages) {
                ...
                if(Objects.nonNull(languageType)) {
                    ...
                    throw new Exception("file2type3feature language is empty:"+language);
                }
            }

            languagesMap.forEach((key, value) -> {
                dataBaseJobService.createFileType3Feature(value, tableCount);
            });
            ...
            try {
                ...
                cacheInfos.forEach(item -> {
                    if(item.getComponent2fileFlag() == ComponentCacheInfo.RUNNING_FLAGS_END &&
                            item.getFile2type3featureFlag() == ComponentCacheInfo.RUNNING_FLAGS_UN_END ) {
    ...
    public void fullComponent() {
        ...
        FileType3FeatureEngineInfo fileFeatureEngineInfo = null;
        try {
            fileFeatureEngineInfo = dataBaseJobService.getFileFeatureEngineInfo();
        }catch (Exception e){
            error_log_.info("file2type3feature模块初始化失败,{}",e.getMessage());
            return;
        }
        ...
        for(int i = index == -1 ? 0 : index;i < languages.size();i++){
            try {
                ...
                int num = Integer.parseInt(fileFeatureEngineInfo.curFileType3Table.substring(table.length()));

                //遍历t_component_file表
                for(int x = num == 0 ? 1 : num;x<=sourceTableCount;x++){
                    fileFeatureEngineInfo.curFileType3Table = String.format("%s%s", table,x);
                    fullComponent(fileFeatureEngineInfo);
                    dataBaseJobService.saveFileFeatureEngineInfo(fileFeatureEngineInfo);
                }
                fileFeatureEngineInfo.curFileType3Table = String.format("%s%s", table, 1);

            }catch (Exception e){
                error_log_.info("遍历指定语言存储表失败,languages = {},table = {},{}",fileFeatureEngineInfo.language,fileFeatureEngineInfo.curFileType3Table,e.getMessage());
            }finally {
                unResources(engineComponent);
            }
        }
    ...
    private void fullComponent(FileType3FeatureEngineInfo engineInfo) {
        List<ComponentCacheInfo> components = null;
        try {
            //获取当前表,指定语言的组件信息
            components = dataBaseJobService.getComponentCaches(engineInfo.curFileType3Table);
        } catch (Exception e) {
            error_log_.info("\n\n ********************* {} 引擎初始化失败 \n\n",
